...
% compute cost function (no regulization)
for i = 1:m
    targ = zeros(num_labels,1);
    targ(y(i)) = 1;
    for k = 1:num_labels
        J = J + (-targ(k)*log(a3(i,k)) - (1-targ(k))*log(1-a3(i,k)))/m;
    end
end
...
for t = 1:m
    % output layer
    targ = zeros(1, num_labels);
    targ(y(t)) = 1;
    delta_3 = a3(t,:) - targ;
    % hidden layer
    delta_2 = Theta2'*delta_3' .* sigmoidGradient([1;Theta1 * a1(t,:)']);
    % accumulate
